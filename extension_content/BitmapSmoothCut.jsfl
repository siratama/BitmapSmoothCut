...
var BitmapSmoothCut = function() {
	...
	while(_g < itemsLength) {
		var i = _g++;
		var item = items[i];
		if(item.itemType != jsfl.ItemType.BITMAP) continue;
		jsfl.Lib.fl.trace(item.name);
		var bitmapItem;
		bitmapItem = js.Boot.__cast(item , BitmapItem);
		bitmapItem.allowSmoothing = false;
		bitmapItem.compressionType = jsfl.CompressionType.LOSSLESS;
	}
	jsfl.Lib.fl.trace("done");
};
BitmapSmoothCut.__name__ = true;
BitmapSmoothCut.main = function() {
	new BitmapSmoothCut();
};
BitmapSmoothCut.prototype = {
	__class__: BitmapSmoothCut
};
var Std = function() { };
Std.__name__ = true;
